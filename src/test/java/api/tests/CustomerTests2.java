...
import api.endpoints.CustomerEndpoint2;
...
public class CustomerTests2 {
	...
	public void retrieveAllCustomers() {
		Response res = CustomerEndpoint2.getAllCustomers();
		res.then().log().all();
	...
	public void testPostCustomers() {
		Response res = CustomerEndpoint2.CreateCustomer(userpayload);
		res.then().log().all();
	...
	public void GetSingleCustomer() {
		Response res = CustomerEndpoint2.getCustomer(this.userpayload.getId());
		res.then().log().all();
		Assert.assertEquals(res.statusCode(), 200);
